/**
 * 节点国家名称标准化脚本 (修复版 v2)
* 用法：在Sub-Store脚本操作中添加
 * 支持参数 (以#开头，多个参数用&连接)：
 *   name=自定义前缀 - 节点添加前缀名称
 *   fgf=分隔符 - 前缀与国家名之间的分隔符 (默认空格)
 * 示例：
 *   https://example.com/script.js#name=MyProxy&fgf=-
 */
...
// 避让列表：如果节点名称包含以下关键词，则被视为"流量/通知/订阅信息"，直接跳过处理
// 防止 100 GB 流量被识别为英国，或日期数字被识别为其他国家
const SKIP_KEYWORDS = [
  '剩余流量', '已用流量', '到期', '过期', '重置', '回国', 
  'Expire', 'Traffic', 'Reset', 'Date', 'Time', 'GB Used', 'GB Total'
];
...
// 辅助函数：生成安全的正则，纯字母数字加边界，其他直接匹配
function getRegex(keyword) {
  // 如果全是字母数字，则强制单词边界 \b，避免 "TH" 匹配到 "Netherlands"
...
// 辅助函数：生成全局正则用于清理
function getGlobalRegex(keyword) {
...
function operator(proxies) {
  ...
  RegionData.forEach(region => {
    region.keywords.sort((a, b) => b.length - a.length);
  });
  const sortedRegionData = RegionData.sort((a, b) => 
    b.keywords[0].length - a.keywords[0].length
  );
  ...
  return proxies.map(proxy => {
    ...
    // 【关键修复】: 检查是否为流量/信息节点
    // 如果名称包含 SKIP_KEYWORDS 中的任意一个词，直接原样返回，不做任何修改
    if (SKIP_KEYWORDS.some(k => originalName.includes(k))) {
      return proxy;
    }
    ...
    // 1. 尝试匹配国家
    for (const region of sortedRegionData) {
      ...
      if (!matchedRegion && region.subKeywords) {
        for (const subKeyword of region.subKeywords) {
          const regex = getRegex(subKeyword);
          if (regex.test(newName)) {
            matchedRegion = region;
            break;
          }
    ...
    // 2. 如果匹配到了国家，执行替换和清理逻辑
    if (matchedRegion) {
      const stdName = `${matchedRegion.emoji} ${matchedRegion.standardName.zh} (${matchedRegion.standardName.en})`;
      
      let cleanedName = originalName;
      // 清理所有该国家的关键词 (如同时删除 "US", "America")
      matchedRegion.keywords.forEach(k => {
        const regex = getGlobalRegex(k);
